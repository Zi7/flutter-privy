...
class TransactionDetailBottomSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final url = 'https://sepolia.etherscan.io/tx/${transaction.hash}';
    return Container(
            ...
            child: Column(
              children: [
                _buildStatusItem(
                  ...
                  title: transaction.type == 'deposit' ? 'Request Deposit' : 'Request Withdraw',
                  description:
                      transaction.type == 'deposit'
                          ? 'Deposit ${transaction.value} ${transaction.symbol}'
                          : 'Withdraw ${transaction.value} ${transaction.symbol}',
                  isCompleted: true,
                  isLast: false,
                ),
                _buildStatusItem(
                  ...
                  title:
                      transaction.type == 'deposit'
                          ? 'Received on ${transaction.symbol}'
                          : 'Sent on ${transaction.symbol}',
                  isCompleted: true,
                  isLast: true,
                ),
              ],
            ...
            child: Column(
              children: [
                _buildDetailRow(
                  ...
                  label: transaction.type == 'deposit' ? 'From' : 'To',
                  value: (transaction.type == 'deposit' ? transaction.from : transaction.to) ?? '',
                  showCopy: true,
                ),
  ...
  Widget _buildStatusItem({
    ...
    return Row(
      ...
      children: [
        Column(
          children: [
            Icon(icon, color: isCompleted ? const Color(0xFF26A17B) : Colors.grey, size: 24),
            if (!isLast)
              Container(
                width: 2,
                height: 40,
  ...
  Widget _buildDetailRow({
    ...
    return Row(
      children: [
        ...
        if (showNetworkIcon) ...[AppSvg.icEth(size: 20), const SizedBox(width: 8)],
        Text(
